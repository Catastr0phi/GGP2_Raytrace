...
// which are necessary pieces for issuing standard API calls
	...
	uploadDesc.Height = 1; // Assuming this is a regular buffer, not a texture
	...
	uploadDesc.Width = cbUploadHeapSizeInBytes; // Size of the buffer
	...
	D3D12_DESCRIPTOR_HEAP_DESC descDesc = {}; // i like this name

	descDesc.Flags = D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE;

	descDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;

	descDesc.NumDescriptors = maxConstantBuffers + MaxTextureDescriptors;

	Device->CreateDescriptorHeap(&descDesc, IID_PPV_ARGS(CBVSRVDescriptorHeap.GetAddressOf()));
...
{
	// Get current fence value and singal into command queue
	...
	// Update current buffer index to the next for the next frame
...

{
...

	{
		...
		void* uploadAddress = (void*)((SIZE_T)cbUploadHeapStartAddress + cbUploadHeapOffsetInBytes);
		...
		if (cbUploadHeapOffsetInBytes > cbUploadHeapSizeInBytes) cbUploadHeapOffsetInBytes = 0;
...

	{
		...
		cbvDescriptorOffset++;
		if (cbvDescriptorOffset >= maxConstantBuffers) cbvDescriptorOffset = 0;
...
unsigned int Graphics::LoadTexture(const wchar_t* file, bool generateMips) {
	// Uploads resource to GPU memory
	...
	// Save descriptor index and advance offset
	unsigned int srvIndex = srvDescriptorOffset;
	srvDescriptorOffset++;

	// Create srv in descriptor heap
	D3D12_CPU_DESCRIPTOR_HANDLE cpuHandle = CBVSRVDescriptorHeap->GetCPUDescriptorHandleForHeapStart();
	cpuHandle.ptr += srvIndex * cbvSrvDescriptorHeapIncrementSize;
